...
def prepare_data(
    ...
    if dataset == "imagenet":
        validation_set = load_dataset(
            "imagenet-1k", split="validation", streaming=True, trust_remote_code=True, cache_dir=path
        )
        train_set = load_dataset(
            "imagenet-1k", split="train", streaming=True, trust_remote_code=True, cache_dir=path
        )
    ...
    # dataset = train_set.map(transform_example)
